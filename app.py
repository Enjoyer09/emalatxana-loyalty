...
from auth import check_url_token_login, validate_session, logout_user, create_session, attempt_login, get_cached_users, get_user_for_auth
from utils import (
...
DEFAULTS = {
    'logged_in': False,
    'session_token': None,
    'multi_carts': {1: {'cart': [], 'customer': None}, 2: {'cart': [], 'customer': None}, 3: {'cart': [], 'customer': None}},
    'active_cart_id': 1,
    'cart_takeaway': [],
        ...
        if not st.session_state.get('handover_mode'):
            ...
            if c_conf1.button("✅ Təsdiqlə və Çıxış Et", type="primary", use_container_width=True):
                ...
                if abs(diff) > Decimal("0.01"):
                    ...
                    actions.append((
                        "INSERT INTO finance (type, category, amount, source, description, created_by, created_at, is_test) VALUES (:t, :c, :a, 'Kassa', 'Smen Təhvili zamanı fərq', :u, :time, FALSE)",
...
st.markdown("""
    ...
    :root { --metal-bg: radial-gradient(circle at center, #4a5159 0%, #1a1d21 100%);
        ...
        --text-light: #ffffff; --border-color: #545b66; }
    ...
    h1, h2, h3 { color: #ffd700 !important; font-family: 'Jura', sans-serif !important; font-weight: 800 !important; }
    div[data-baseweb="input"] > div { background-color: #16191d !important; border: 2px solid #3a4149 !important; border-radius: 8px !important; }
    div[data-baseweb="input"] input { color: #ffffff !important; font-weight: 700 !important; }
    button[kind="primary"] { background: var(--accent-gold) !important; border: 2px solid #ffd700 !important; border-radius: 10px !important; }
    button[kind="primary"] p { color: #000000 !important; font-weight: 900 !important; }
    button[kind="secondary"] { background: var(--metal-btn) !important; border: 2px solid #3a4149 !important; border-radius: 10px !important; }
    button[kind="secondary"] p { color: #ffffff !important; font-weight: 800 !important; }
    .stMetric { background: var(--metal-btn); padding: 15px; border-radius: 10px; border: 2px solid #3a4149; }
    .stMetric label { color: #aaa !important; } .stMetric div { color: #ffd700 !important; font-family: 'Jura'; }
    .pin-box { background-color: #16191d !important; border: 2px solid #3a4149 !important; border-radius: 8px !important;
        height: 50px; display: flex; align-items: center; justify-content: center; color: #ffd700 !important;
        font-size: 32px !important; letter-spacing: 15px; margin-bottom: 15px; }
    </style>
...
if not st.session_state.logged_in:
    ...
    with c2:
        ...
        with t1:
            ...
            disp = "• " * len(st.session_state.staff_pin) if st.session_state.staff_pin else \
                "<span style='color:#7b8896; font-size:16px;'>PIN DAXİL EDİN</span>"
            ...
            if st.button("Giriş", type="primary", use_container_width=True, key="staff_login_btn"):
                # Try all staff/manager users
                all_users = run_query("SELECT username, password, role FROM users WHERE role IN ('staff', 'manager')")
                found = False
                for _, r in all_users.iterrows():
                    success, token, error_msg = attempt_login(r['username'], st.session_state.staff_pin)
                    if success:
    ...
    selected_tab = st.radio("Menu", tabs_list, horizontal=True, label_visibility="collapsed", key="main_nav_radio", index=idx)
