...
def generate_pdf(cart, total):
    ...
    c.drawCentredString(100, y, APP_NAME); y-=20
    for i in cart:
...
with st.container():
    ...
    for _, m in menu.iterrows():
        cat = m["category"]
        cls = "coffee" if cat=="Qəhvə" else "drink" if cat=="İçkilər" else "dessert"
        qty = get_qty(m["id"])
        ...
        <div class="pos-card {cls}">
          {badge}
          <div class="pos-icon">{EMOJI.get(cat,"•")}</div>
