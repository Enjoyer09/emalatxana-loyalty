...
# --- INFRASTRUKTUR AYARLARI (YENİLƏNDİ: IRONWAVES & RESEND) ---
...
# --- DATABASE CONNECTION (DÜZƏLDİLDİ) ---
try:
    db_url = os.environ.get("STREAMLIT_CONNECTIONS_NEON_URL")
    if not db_url: st.error("URL yoxdur!"); st.stop()
    # Postgres URL fix for SQLAlchemy
    if db_url.startswith("postgres://"):
        db_url = db_url.replace("postgres://", "postgresql+psycopg2://", 1)
    
    # URL birbaşa arqument kimi ötürülür
    ...
    for _ in range(3):
        try:
            with conn.session as s:
                ...
                try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS email TEXT;"))
                except: pass
                try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS birth_date TEXT;"))
                except: pass
                try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT FALSE;"))
                except: pass
                try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS last_visit TIMESTAMP;"))
                except: pass
                try: s.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS secret_token TEXT;"))
                except: pass
...
# --- EMAIL SYSTEM (RESEND-ə KEÇİRİLDİ) ---
def send_email(to_email, subject, body):
    ...
    payload = {
        ...
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; border: 1px solid #eee; border-radius: 10px;">
            <h2 style="color: #2E7D32;">{SENDER_NAME}</h2>
            ...
            with st.form("act"):
                ...
                with st.expander("📜 İstifadəçi Razılaşmasını Oxu"):
                    ...
                    **EMALATXANA COFFEE — İSTİFADƏÇİ RAZILAŞMASI**
                    1. **Məxfilik:** E-mail və Doğum Tarixi yalnız kampaniyalar üçündür.
                    2. **Sadiqlik:** 9 ulduz = 1 pulsuz kofe.
                    3. **Termos:** Öz termosu ilə gələnlərə endirim var.
                    4. **Ad Günü:** Hədiyyə üçün şəxsiyyət vəsiqəsi təqdim edilməlidir.
                    """)
...
# ========================
    ...
    if not st.session_state.logged_in:
        ...
        if role == 'admin':
            tabs = st.tabs(["POS", "Analitika", "CRM", "Menyu", "⚙️ Admin", "QR"])
                ...
                if not m_df.empty:
                    ...
                    if st.button("🚀 Seçilənləri Göndər", key="crm"):
                        ...
                        for i, r in ed.iterrows():
                            ...
                            if r['Ad Günü']: 
                                if send_email(r['email'], f"{SHOP_NAME}: Ad Gününüz Mübarək!", "Bir kofe bizdən hədiyyə!"):
                            ...
                            if r['Pulsuz Peceniya']: 
                                if send_email(r['email'], f"{SHOP_NAME}: Şirin Hədiyyə!", "Kofe alana Peceniya bizdən!"):
            ...
            with tabs[4]:
                st.markdown("### ⚙️ Ayarlar & Admin")
                
                # --- YENİ BÖLMƏ: İŞÇİ İDARƏETMƏSİ (ADD/REMOVE/RESET) ---
                with st.expander("👥 İşçilər (İstifadəçi İdarəetməsi)", expanded=True):
                    ...
                    with c1:
                        ...
                        if st.button("Əlavə Et"):
                            if len(new_p) < 6: st.error("Şifrə qısadır")
                            else:
                                try:
                    ...
                    with c2:
                        ...
                        if target:
                            ...
                            new_pass_reset = st.text_input("Yeni Şifrə", type="password", key="res_pass")
                            if st.button("Yenilə"):
                                run_action("UPDATE users SET password=:p WHERE username=:u", {"p":hash_password(new_pass_reset), "u":target})
