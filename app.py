...
def send_email(to_email, subject, body):
    ...
    payload = {
        ...
        "html": f"<div style='font-family:Arial; padding:20px; color:#333; border:1px solid #eee; border-radius:10px;'><h2>{SHOP_NAME}</h2><hr><p>{body.replace(chr(10), '<br>')}</p><hr><small>IronWaves Loyalty</small></div>"
    }
    try:
                ...
                if not m_df.empty:
                    ...
                    if st.button("🚀 Göndər"):
                        ...
                        for index, row in edited_df.iterrows():
                            if row['Seç']: send_email(row['email'], "Xüsusi Təklif", "Sizi gözləyirik!"); cnt+=1
