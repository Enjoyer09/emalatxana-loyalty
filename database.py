...
def run_query(q, p=None): 
    ...
    # Neon bağlantısı üçün parametr idarəetməsi
...
def set_setting(key, value): 
    run_action("INSERT INTO settings (key, value) VALUES (:k, :v) ON CONFLICT (key) DO UPDATE SET value=:v", {"k":key, "v":str(value)})

@st.cache_resource
def ensure_schema():
    if not conn: return False
    with conn.session as s:
        # SƏNİN BÜTÜN ORİJİNAL CƏDVƏLLƏRİN (HEÇ BİRİ SİLİNMƏYİB)
        ...
        s.execute(text("CREATE TABLE IF NOT EXISTS logs (id SERIAL PRIMARY KEY, user_name TEXT, action TEXT, created_at TIMESTAMP);"))
        
        # Növbə idarəetməsi başlanğıcı
