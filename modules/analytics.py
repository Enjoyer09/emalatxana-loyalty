...
def render_analytics_page():
    ...
    if not sales.empty:
        ...
        # BANK KOMİSSİYASI (2%) VƏ NET HESABLAMA
        bank_fee_rate = 0.02 
        ...
        c3.metric("Nağd (Kassa)", f"{real_sales[real_sales['payment_method']=='Cash']['total'].sum():.2f} ₼")
        ...
        with st.expander("💸 GÜNLÜK MAAŞ/AVANS"):
            with st.form("sal_f_p2"):
                ...
                if st.form_submit_button("💰 Ödə"):
                    run_action("INSERT INTO finance (type, category, amount, source, created_by) VALUES ('out', 'Maaş', :a, 'Kassa', :u)", {"a":amt, "u":st.session_state.user})
