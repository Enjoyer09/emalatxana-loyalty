...
def show_refund_dialog(sale_id):
    ...
    if st.button("✅ LƏĞVİ TƏSDİQLƏ", type="primary", use_container_width=True, key="confirm_refund"):
        ...
        if is_full_void:
            actions.append(("UPDATE sales SET status='VOIDED' WHERE id=:id", {"id": sale_id}))
        else:
            actions.append(("UPDATE sales SET status='PARTIAL_REFUND' WHERE id=:id", {"id": sale_id}))
            ...
            try:
                ...
                for item in parsed:
                    ...
                    for _, rc in recs.iterrows():
                        ...
                        actions.append(("UPDATE ingredients SET stock_qty = stock_qty + :q WHERE name=:n", {"q": str(qty_return), "n": rc['ingredient_name']}))
        ...
        if not is_test and row.get('customer_card_id') and is_full_void:
            try:
                ...
                if coffee_count > 0:
                    actions.append(("UPDATE customers SET stars = GREATEST(0, stars - :s) WHERE card_id=:cid", {"s": coffee_count, "cid": row['customer_card_id']}))
            except:
                pass
...
def render_analytics_page():
    ...
    if not sales.empty:
        ...
        with tab1:
            ...
            sales_disp['Status'] = sales_disp['status'].apply(lambda x: '🔴 LƏĞV' if x == 'VOIDED' else '🟡 QİSMƏN' if x == 'PARTIAL_REFUND' else '✅')
                ...
                if not s_res.empty:
                    ...
                    def edit_dialog(r):
                        with st.form("ed_f"):
                            ...
                            pm_options = ["Nəğd", "Kart", "Staff", "Cash", "Card", "Split"]
                            ...
                            if st.form_submit_button("Yadda Saxla"):
                                run_action("UPDATE sales SET total=:t, payment_method=:p WHERE id=:id", {"t": str(Decimal(str(e_t))), "p": e_p, "id": r['id']})
        ...
        with tab3:
            ...
            if not refunds.empty:
                ...
                if st.session_state.role == 'admin':
                    ...
                    by_user = refunds.groupby('created_by').agg(sayi=('id', 'count'), meblegi=('refund_amount', 'sum')).sort_values('sayi', ascending=False)
            ...
            if st.session_state.role in ['admin', 'manager']:
                ...
                if not staff_stats.empty:
                    ...
                    void_stats = run_query(
                        "SELECT created_by, COUNT(*) as cnt, SUM(refund_amount) as total_refund FROM refunds WHERE created_at BETWEEN :s AND :e GROUP BY created_by",
                        {"s": ts_start, "e": ts_end}
                    )
...
def render_z_report_page():
    ...
    if st.session_state.role == 'staff':
        ...
        split_count = 0
        try:
            sc = run_query(f"SELECT COUNT(*) as c FROM sales WHERE payment_method LIKE 'Split%' {q_cond}", params)
            split_count = sc.iloc[0]['c'] or 0
        except:
            pass
        ...
        if st.session_state.role in ['admin', 'manager']:
            ...
            if split_count > 0:
                st.caption(f"📊 Bu növbədə **{split_count}** bölünmüş ödəniş var")
