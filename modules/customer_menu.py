# modules/customer_menu.py — FINAL PATCHED v5.0
...
def inject_customer_css():
    ...
    .app-logo { font-weight: 900; font-size: 22px; color: #F5E6D3; letter-spacing: 3px; }
    ...
    .sc-title { font-weight: 900; font-size: 17px; color: #3C2415; }
    .sc-tier {
    ...
    .sc-subtitle { font-size: 12px; color: #AAA; margin-bottom: 18px; }
    ...
    .sc-footer-text { text-align: center; font-size: 12px; color: #AAA; font-weight: 600; }
    ...
    .rw-icon { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; }
    .rw-icon-coffee { background: #F0E4D7; }
    .rw-icon-gift { background: #FFF3E0; }
    ...
    .rw-pct { font-weight: 900; font-size: 16px; color: #3C2415; }
    ...
    .qr-wrap { padding: 0 16px; margin-bottom: 14px; }

    .menu-cat-title {
        font-size: 13px; font-weight: 900; color: #3C2415;
    ...
    .cg-emoji { font-size: 30px; margin-bottom: 6px; }
    ...
    .cg-stamp { font-size: 10px; color: #E88D48; font-weight: 700; margin-top: 4px; background: #FFF3E0; padding: 2px 6px; border-radius: 8px; display: inline-block; }
    ...
    .ml-price { font-weight: 900; font-size: 17px; color: #3C2415; white-space: nowrap; margin-left: 10px; }
    ...
    .h-stamp { display: inline-block; background: #F0E4D7; color: #3C2415; padding: 2px 8px; border-radius: 8px; font-size: 10px; font-weight: 800; margin-top: 4px; }
    ...
    button[kind="primary"], button[kind="primaryFormSubmit"] {
        background: linear-gradient(135deg, #3C2415, #5C3A28) !important;
        border: none !important; border-radius: 14px !important;
        box-shadow: 0 4px 15px rgba(60,36,21,0.25) !important;
        min-height: auto !important;
    }
    button[kind="primary"] p, button[kind="primaryFormSubmit"] p { color: #F5E6D3 !important; font-weight: 900 !important; font-size: 15px !important; }

    button[kind="secondary"], button[kind="secondaryFormSubmit"] {
        background: #FFF !important; border: 1px solid #EDE8E3 !important;
    ...
    .stTabs [data-baseweb="tab-list"] { gap: 0; background: #FFF; border-radius: 14px; padding: 4px; margin: 0 16px 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); border: 1px solid #EDE8E3; }
...
def render_stamp_card(stars, cust_type):
    ...
    <div class="stamp-card">
        ...
        <div class="sc-footer-text">☕ {filled}/10 stamp · Pulsuz kofeyə {remaining} qaldı</div>
    </div>
    {free_html}
    """, unsafe_allow_html=True)
...
def render_coffee_menu():
    ...
    if "Hamısı" in sel:
            ...
            for idx in range(0, len(items), 2):
                ...
                for j in range(2):
                    if idx + j < len(items):
                        ...
                        stamp = '<div class="cg-stamp">+1 Stamp ☕</div>' if item['is_coffee'] else ''
                        html += f'''
                        <div class="cg-item">
        ...
        for _, item in filtered.iterrows():
            star = '<span style="color:#E88D48; margin:0 6px;">⭐</span>' if item['is_coffee'] else ""
            st.markdown(f"""
            <div class="ml-item">
                <div>
...
def render_history(card_id):
    ...
    for _, row in sales.iterrows():
        ...
        stamp_html = f'<div class="h-stamp">+{coffee_count} stamp ☕</div>' if coffee_count > 0 else ""
...
Azərbaycan dilində, emoji ilə, sirli üslubda."""
                    ...
                    <div class="fr">
                        ...
                        <div style="text-align:center; font-weight:900; color:#3C2415; margin-bottom:12px; font-size:16px;">SİZİN FALINIZ</div>
