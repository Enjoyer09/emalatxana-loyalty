                ...
                if st.form_submit_button("Əlavə Et") and mn_name and pack_size > 0:
                    try:
                        ...
                        log_system(
                            ...
                            "INVENTORY_ADD",
                            {
                                "item_name": mn_name.strip(),
                                "total_qty": str(total_qty),
                                "unit": mn_unit,
                                "unit_cost": str(unit_cost),
                                "category": mn_cat_final
                            }
                        )
                        st.success("✅ OK")
                        time.sleep(1)
    ...
    if not df_page.empty:
        ...
        if len(sel_ids) > 0 and c4_btn.button("🗑️ Sil", key="anbar_del_btn", use_container_width=True):
            for i in sel_ids:
                ...
                log_system(st.session_state.user, "INVENTORY_DELETE", {"item_name": item_name, "item_id": int(i)})
            st.success("Silindi!")
            time.sleep(1)
            st.rerun()
                ...
                with st.form("loss_form"):
                    ...
                    if st.form_submit_button("Təsdiqlə və Sil", type="primary"):
                        if not reason.strip():
                            ...
                            if loss_amt > 0:
                                actions.append((
                                    "INSERT INTO finance (type, category, amount, source, description, created_by, created_at, is_test) VALUES ('out', 'Anbar İtkisi', :a, 'Anbar', :d, :u, :t, FALSE)",
                                    {"a": str(loss_amt), "d": f"{r['name']} - {reason}", "u": st.session_state.user, "t": get_baku_now()}
                                ))
                            ...
                            try:
                                ...
                                log_system(
                                    ...
                                    "INVENTORY_LOSS",
                                    {
                                        "item_id": int(r['id']),
                                        "item_name": r['name'],
                                        "qty_removed": str(qty_d),
                                        "loss_amount": str(loss_amt),
                                        "reason": reason
                                    }
                                )
                                st.session_state.loss_item_id = None
                                st.success(f"Məxaric edildi və {loss_amt:.2f} ₼ şirkət zərəri kimi qeyd olundu!")
                                time.sleep(1.5)
                                st.rerun()
                            except Exception as e:
                                st.error(f"Xəta: {e}")
