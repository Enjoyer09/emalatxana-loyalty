...
def show_receipt_dialog(cart_data, total_amt):
    ...
    <div id="receipt_area" style="width: 280px; padding: 10px; font-family: 'Courier New', monospace; color: black; background: white; border: 1px solid #eee;">
        ...
        <p style="text-align: center; font-size: 9px; color: #666;">v1.7 Master Patch</p>
    </div>
...
def z_report_dialog():
    ...
    if st.button("✅ Günü Bağla və Maaşı Çıxar", use_container_width=True, key="z_confirm_btn"):
        ...
        run_action("INSERT INTO finance (type, category, amount, source, description, created_by, subject, created_at, is_test) VALUES ('out', 'Maaş/Avans', :a, 'Kassa', 'Smen sonu maaş', :u, :subj, :time, :tst)", {"a": wage_amt, "u": u, "subj": u, "time": now, "tst": is_t})
        run_action("INSERT INTO finance (type, category, amount, source, description, created_by, created_at, is_test) VALUES ('in', 'Kassa Açılışı', :a, 'Kassa', 'Sabah üçün açılış balansı', :u, :time, :tst)", {"a": next_open, "u": u, "time": now, "tst": is_t})
...
def calculate_smart_total(cart, customer=None, is_table=False, manual_discount_percent=0, is_eco_cup=False):
    ...
    if is_table and service_fee_pct > 0: final_total += final_total * service_fee_pct
    if is_eco_cup: final_total *= 0.95 
...
def render_menu(cart, key):
    ...
    cats = ["HAMISI"] + [c.upper() for c in sorted(menu_df['category'].unique().tolist(), key=lambda x: CAT_ORDER_MAP.get(x, 99))]
    sc_upper = st.radio("Kat", cats, horizontal=True, label_visibility="collapsed", key=f"c_rad_{key}")
    sc = "Hamısı" if sc_upper == "HAMISI" else next((c for c in menu_df['category'].unique() if c.upper() == sc_upper), "Hamısı")
...
def render_pos_page():
    ...
    with c_carts[3]:
        if st.button("📊 Günü Bitir (Z)", key="z_report_trigger_btn", use_container_width=True, type="primary"):
            st.session_state.active_dialog = ("z_report", None); st.rerun()
    ...
    with c_cart:
        with st.container(border=True):
            ...
            if cust: 
                ...
                c_h.success(f"👤 {cust['card_id']} | ⭐ {cust['stars']}")
                c_d.button("❌", key="cust_clear_btn", on_click=clear_customer_data_callback)
            ...
            if not st.session_state.cart_takeaway: st.markdown("<p style='text-align:center; color:#888;'>Səbət boşdur</p>", unsafe_allow_html=True)
                ...
                for i, item in enumerate(st.session_state.cart_takeaway):
                    c_n, c_q, c_bc = st.columns([4, 1, 2], vertical_alignment="center")
                    c_n.markdown(f"**{item['item_name']}**")
                    c_q.write(f"x{item['qty']}")
                    with c_bc:
                        b_m, b_p = st.columns(2)
                        if b_m.button("➖", key=f"c_dec_{i}"): 
                             if item['qty'] > 1: item['qty'] -= 1
                             else: st.session_state.cart_takeaway.pop(i)
                             st.rerun()
                        if b_p.button("➕", key=f"c_inc_{i}"): item['qty'] += 1; st.rerun()
            ...
            if pm == "Staff":
                ...
                for it in st.session_state.cart_takeaway:
                    if it.get('is_coffee'): staff_consumed += (it['qty'] * it['price'])
                    else:
            ...
            if st.button("✅ ÖDƏNİŞİ TAMAMLA", type="primary", use_container_width=True, key="cart_pay_btn"):
                ...
                try:
                    with conn.session as s:
                        ...
                        s.commit(); log_system(st.session_state.user, f"{'🧪 TEST ' if is_test_mode else ''}SATIŞ: {final:.2f} AZN")
                    ...
                    st.session_state.search_key_counter += 1
                    st.session_state.active_dialog = ("receipt", receipt_data); st.rerun()
