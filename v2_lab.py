...
# ==========================================
# === IRONWAVES POS - HYBRID V1 + V2 (INVENTORY) ===
# ==========================================
...
# ==========================================
# === DİZAYN KODLARI (CSS & JS - V1 ORIGINAL) ===
# ==========================================
st.markdown("""
    <script>
...
def verify_password(p, h): 
    try:
        if h.startswith('$2b$'): return bcrypt.checkpw(p.encode(), h.encode())
        return p == h
    except: return False
        ...
        with conn.session as s:
            for item in items_list:
                ...
                if recipes:
                    ...
                    log_msgs.append(f"✅ {item_name}: Stokdan çıxıldı.")
                else:
                    log_msgs.append(f"⚠️ {item_name}: Resept yoxdur.")
            s.commit()
    except Exception as e:
        log_msgs.append(f"❌ Xəta: {e}")
    return log_msgs
...
def send_email(to_email, subject, body):
    ...
    try: r = requests.post(url, json=payload, headers=headers); return r.status_code == 200
    except: return False
...
if "id" in query_params:
    ...
    if not df.empty:
            ...
            with st.form("act"):
                ...
                with st.expander("📜 Qaydalar və İstifadəçi Razılaşması"):
                    ...
                    <div style="font-size:14px; color:#333; line-height: 1.6;">
                        ...
                        <b>6. Məxfilik Siyasəti:</b> İstifadəçinin şəxsi məlumatları yalnız fərdi kampaniyalar məqsədilə istifadə olunur.<br>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        if st.button("🔴 Hesabdan Çıx", type="primary"): st.query_params.clear(); st.rerun()

    else: st.error("Kart tapılmadı")

# ========================
# === 2. POS & ADMIN (V1 + V2 Inventory Tab) ===
# ========================
else:
    ...
    if st.session_state.logged_in:
        ...
        with st.sidebar:
            ...
            try:
                user_stats = run_query("SELECT username, last_seen FROM users ORDER BY username")
                for _, u_row in user_stats.iterrows():
                    is_online = False
                    if u_row['last_seen']:
                    ...
                    <div style="display:flex; align-items:center; margin-bottom:10px;">
                        ...
                        <span style="font-size:16px;">{u_row['username']} <span style="color:#777; font-size:12px;">({lbl})</span></span>
                    </div>
                    """, unsafe_allow_html=True)
            except: pass
            ...
            with layout_col1:
                ...
                if st.button("✅ ÖDƏNİŞ ET", type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        # --- V2 TRIGGER: ANBARDAN SİL ---
                        deduct_msgs = deduct_inventory_for_sale(st.session_state.cart)
                        for msg in deduct_msgs: st.toast(msg)
                ...
                for base_name, items in groups.items():
                    with cols[i % 4]:
                        if len(items) > 1:
                            ...
                            if st.button(f"☕ {items[0]['item_name']}" if items[0]['is_coffee'] else f"{items[0]['item_name']}\n{items[0]['price']}₼", key=f"itm_{items[0]['id']}"): st.session_state.cart.append(items[0]); st.rerun()
                    i += 1

        if role == 'admin':
            tabs = st.tabs(["POS", "📦 Anbar (V2)", "Analitika", "CRM", "Menyu", "⚙️ Ayarlar", "Admin", "QR"])
            with tabs[0]: render_pos()
            
            # --- V2 ANBAR TABI ---
            with tabs[1]:
                st.markdown("### 📦 Anbar və Resept İdarəetməsi")
                sub_t1, sub_t2 = st.tabs(["Xammal (Inventory)", "Reseptlər"])
                
                with sub_t1:
                    with st.expander("➕ Yeni Xammal"):
                        with st.form("new_inv_admin"):
                            c1, c2 = st.columns(2)
                            n = c1.text_input("Ad"); u = c2.selectbox("Vahid", ["litr","kq","eded","qr"])
                            s = c1.number_input("Stok", 0.0); c = c2.number_input("Vahid Qiyməti", 0.0)
                            if st.form_submit_button("Əlavə Et"):
                                run_action("INSERT INTO inventory (name, unit, stock_level, cost_per_unit) VALUES (:n,:u,:s,:c)", {"n":n,"u":u,"s":s,"c":c})
                                st.success("OK"); st.rerun()
                    
                    df_inv = run_query("SELECT * FROM inventory ORDER BY id")
                ...
                with sub_t2:
                    ...
                    with c2:
                        ...
                        sel_inv = st.selectbox("Xammal:", list(inv_dict.keys()) if inv_dict else [])
                    
                    qty = st.number_input("Miqdar", 0.001, format="%.3f")
                    if st.button("Reseptə Əlavə Et"):
                        if sel_menu and sel_inv:
                            ...
                            st.success("Resept yeniləndi!")
                    
                    st.divider()
                    rec_view = run_query("SELECT r.menu_item_name, i.name as xammal, r.quantity_required, i.unit FROM recipes r JOIN inventory i ON r.inventory_item_id = i.id ORDER BY r.menu_item_name")
            ...
            with tabs[2]:
                ...
                if not sales.empty:
                    ...
                    st.dataframe(sales[['id', 'created_at', 'items', 'total', 'payment_method', 'cashier']], hide_index=True)
                    ...
                    with st.form("del_sale"):
                        ...
                        if st.form_submit_button("Sil"):
                            ...
                            if not adm.empty and verify_password(apass, adm.iloc[0]['password']):
                                ...
                                st.success(f"Satış #{sid} silindi!"); time.sleep(1); st.rerun()
                        ...
                        if st.form_submit_button("Seçilənlərə Göndər"):
                            ...
                            if not selected_rows.empty:
                                ...
                                for idx, row in selected_rows.iterrows():
                                    email = row['email']; cid = row['card_id']
            ...
            with tabs[4]:
                ...
                with st.form("add"):
                    c1,c2,c3 = st.columns(3)
                    n = c1.text_input("Ad"); p = c2.number_input("Qiymət", min_value=0.0, step=0.1); c = c3.selectbox("Kat", ["Qəhvə","İçkilər","Desert"]); cf = st.checkbox("Kofedir?")
                    if st.form_submit_button("Əlavə Et"):
                        if n and n.strip(): 
                            run_action("INSERT INTO menu (item_name, price, category, is_coffee) VALUES (:n,:p,:c,:ic)", {"n":n,"p":p,"c":c,"ic":cf})
                            st.success("Əlavə olundu!"); st.rerun()
                        else: st.error("Ad boş ola bilməz!")
                ...
                if not m_list.empty:
                    ...
                    del_id = st.selectbox("Silmək üçün:", m_list['id'].astype(str) + " - " + m_list['item_name'], index=None)
                    if del_id and st.button("🗑️ Sil", type="primary"):
                        run_action("DELETE FROM menu WHERE id = :id", {"id": del_id.split(" - ")[0]})
                ...
                if st.button("📥 BÜTÜN BAZANI YÜKLƏ (BACKUP)", type="primary"):
                    try:
                        ...
                        with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                            ...
                            clean_df_for_excel(run_query("SELECT * FROM inventory")).to_excel(writer, sheet_name='Inventory')
